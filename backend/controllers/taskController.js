...
export const getUserTasks = async (req, res) => {
  try {
    const tasks = await Task.find({ userId: req.user.id })
      .populate("selectedProviderId", "name email averageRating")
      .sort({ createdAt: -1 });
    res.json(tasks);
  } catch (error) {
    res.status(500).json({ message: error.message });
...
export const completeTask = async (req, res) => {
  try {
    ...
    if (task.selectedProviderId.toString() !== req.user.id) {
      return res.status(403).json({ message: "Only the assigned provider can complete this task" });
    }
...
export const rateProvider = async (req, res) => {
  try {
    ...
    if (task.userId.toString() !== req.user.id) {
      return res.status(403).json({ message: "Only the task owner can rate the provider" });
    }
    ...
    if (provider) {
      ...
      const newAverageRating = ((provider.averageRating * provider.totalRatings) + score) / newTotalRatings;
